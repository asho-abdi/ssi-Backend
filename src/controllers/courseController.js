...
/** Collect every ImageKit fileId on a course for cleanup when the course is deleted. */
function collectImageKitFileIdsFromCoursePlain(plain) {
  ...
  const scanResources = (arr) => {
    for (const r of arr || []) pushId(r?.imagekit_file_id);
  };
  scanResources(plain.all_resources);
  for (const topic of plain.course_topics || []) scanResources(topic.resources);
  for (const row of plain.topic_resources || []) scanResources(row.resources);
  return ids;
}

/**
 * Proxy download: after auth, stream bytes from the stored HTTPS URL (ImageKit delivery)
 * so the browser still uses the authenticated /courses/.../download route.
 */
...
async function downloadCourseResource(req, res) {
  ...
  if (fileUrl.startsWith('https://') || fileUrl.startsWith('http://')) {
    try {
      ...
      console.error('[course] resource stream failed:', err?.message || err);
      if (!res.headersSent) res.status(502).json({ message: 'Download failed' });
    }
    return;
  }

  return res.status(404).json({ message: 'Resource URL missing; the file may need to be re-uploaded.' });
}

async function listCourses(req, res) {
  const courses = await Course.find()
    .populate('teacher_id', 'name email avatar_url')
...
async function createCourse(req, res) {
  ...
  const course = await Course.create({
    ...
    thumbnail_file_id: req.body.thumbnail_file_id ? String(req.body.thumbnail_file_id).trim() : '',
    video_url,
...
async function updateCourse(req, res) {
  ...
  if (thumbnail != null) {
    ...
    const incomingFileId =
      thumbnail_file_id !== undefined ? String(thumbnail_file_id || '').trim() : undefined;

    const thumbUrlChanged = nextThumb !== prevThumb;
    const fileIdWillChange = incomingFileId !== undefined && incomingFileId !== prevFileId;

    /** Replacing or clearing the thumbnail ImageKit object before saving the new ids/urls. */
    if (prevFileId && (thumbUrlChanged || fileIdWillChange)) {
...
async function deleteCourse(req, res) {
  ...
  const plain = course.toObject({ flattenMaps: true });
  const fileIds = collectImageKitFileIdsFromCoursePlain(plain);
  for (const fid of fileIds) {
    await safeDeleteFile(fid);
  }
